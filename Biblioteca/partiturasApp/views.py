...
from django.urls import reverse_lazy    #Dia 13, creando, actualizando por clases.
...
"""DIA 13 - Actualizar vistas FBV a CBV (De funciones a Clases)"""

#Este era la FBV de lista_partituras
# def lista_partituras(request):
#     '''Mi primera CBV refactorizada desde una FBV'''
#     partituras = Partitura.objects.all()
#     contexto = {'partituras':partituras}
#     return render(request, 'partiturasApp/partitura/lista_partituras.html', contexto)

#nueva CBV refactorizada desde la funcion anterior.

class PartituraListView(ListView):
    model = Partitura
    # Por convención, Django buscará una plantilla llamada:
    # <nombre_app>/<modelo>_list.html -> partituras/partitura_list.html
    # Y la variable en el contexto se llamará 'object_list'
    #PERO le decimos esta vez cual usar
    template_name = 'partiturasApp/partitura/lista_partituras.html'
    # y como se llamara la variable con el objeto del contexto
    context_object_name = 'partituras'
    
    def get_queryset(self):
        return Partitura.objects.filter()   #este Queryset sirve para aplicar un filtro a la clase, solo se muestran los objetos que cumplen la condicion.
    

    
"""Creando mi primera CBV sin ayuda"""

class CompositorListView(ListView): #funciono
    model = Compositor
    template_name = 'partiturasApp/compositor/lista_compositores.html'
    context_object_name = 'compositores'


"""Creando el details"""
class PartituraDetalle(DetailView):
    model = Partitura
    template_name = 'partiturasApp/partitura/detalle_partitura.html'
    # Por convención, la variable en el contexto se llama 'object' o 'partitura'
    # Funciona solo por convencion, pero pondre el nombre del contexto para ayudar no mas, sera partitura.
    # DetailView automáticamente hace el get_object_or_404 usando el pk de la URL.
    context_object_name = 'partitura'
    
class CompositorDetalle(DetailView):
    model = Compositor
    template_name = 'partiturasApp/compositor/detalle_compositor.html'
    context_object_name = "compositor"


class PartituraCreate(CreateView):
    model = Partitura
    form_class = PartituraForm
    template_name = 'partiturasApp/partitura/crear_partitura.html'
    
class CompositorCreateView(CreateView):
    model = Compositor
    form_class = CompositorForm
    template_name = "partiturasApp/compositor/crear_compositor.html"
    
class PartituraUpdateView(UpdateView):
    model = Partitura
    form_class = PartituraForm
    template_name = "partiturasApp/partitura/edit_partitura.html"
    context_object_name = 'partitura'

    success_url = reverse_lazy('lista_partituras')

    
class CompositorUpdateView(UpdateView):
    model = Compositor
    form_class = CompositorForm
    template_name = "partiturasApp/compositor/edit_compositor.html"
    context_object_name = 'compositor'
    
    success_url = reverse_lazy('lista_compositores')
    

class PartituraDeleteView(DeleteView):
    model = Partitura
    template_name = "partiturasApp/partitura/borrar_partitura.html"
    context_object_name = 'partitura'
    success_url = reverse_lazy('lista_partituras')

class CompositorDeleteView(DeleteView):
    model = Compositor
    template_name = "partiturasApp/compositor/borrar_compositor.html"
    context_object_name = 'compositor'
    success_url = reverse_lazy('lista_compositores')
